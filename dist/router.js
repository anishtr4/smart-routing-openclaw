...
exports.explainRouting = explainRouting;
...
function routeRequest(messages, defaultTier = 'MEDIUM') {
    ...
    if (reasoningCount >= 2) {
        const model = (0, models_1.getCheapestModelForTier)('REASONING');
        return {
            model,
            tier: 'REASONING',
            confidence: 0.97,
    ...
    let tokenScore = 0;
    if (tokens < 50)
        tokenScore = -DEFAULT_WEIGHTS.tokenCount; // Very short = likely simple
    else if (tokens > 500)
        tokenScore = DEFAULT_WEIGHTS.tokenCount; // Very long = likely complex
    // Question complexity
    ...
    const totalScore = scores.reasoning +
        scores.code +
        scores.multiStep +
        scores.technical +
        scores.creative +
    ...
    const model = (0, models_1.getCheapestModelForTier)(tier);
    return {
        model,
        tier,
        confidence,
...
function explainRouting(decision) {
